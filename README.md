...
サービスイン後は運用業務をそのまま担当した。

#### 技術的な話
1つ目のサービスではLINE Blockchainを使用していたが、不安定さがあり2つ目のサービスでは採用しなかった。
サービス共通の機能を一元管理したいという要求から、独自にスマートコントラクト(ERC1155)を実装する方式を採用した。Layer2の利用はPolygonを使用。
...
APIサーバはすべてGoで実装。LambdaはNodejsやGoなど様々な言語で実装されていた。

ユーザーデータはすべてDynamodbで保存。マスターデータはRDBで管理。認証セッションやキャッシュ用途でRedisを使用。
...
アプリケーションでは「トランザクション発行後、PolygonScanやArbitrumNovaなどexplorer上でトランザクションの成功ログを確認できる」まででNFT転送成功と見なしていた。
タイムアウトは設定していたが、一連のフローを同期的に判定している箇所もあり、APIによってはレスポンスタイムが数十秒かかることがあった。
オンチェインとオフチェインの同期機構は用意しており、最終的にはブロックチェーンとサービス内では結果が整合するよう設計していた。

#### 役割とコラボレーション
プロジェクト内では手を動かす役割だけではなく、課題解決のため下記のような行動もした
